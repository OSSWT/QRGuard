...
def _structural_source_rows() -> dict[tuple[str, str], dict[str, str]]:
    ...
    candidates = sorted(
        ...
        key=lambda row: (split_priority.get(row["split"], 9), row["sample_path"]),
    )
...
def _structural_cases() -> list[DemoCase]:
    ...
    for label, prefix, intended in LABELS:
        for condition, recorded_condition in CONDITIONS:
            ...
            source = STRUCTURAL_CAPTURE_ROOT / row["sample_path"]
            ...
            if payload_hash != row["payload_hash"]:
                raise RuntimeError(
                    f"Payload contract changed for {row['capture_session']}"
                )
            cases.append(
                DemoCase(
                    case_id=output.stem,

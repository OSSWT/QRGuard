...
Pipeline:
  ...
  7. Save a versioned candidate. Promotion to the runtime weights requires both
     passing gates and an explicit ``--promote`` command.
...
Usage:
    python scripts/train_fusion.py --structural-artifacts <v3-artifacts>
...
from structural.image_quality import (  # noqa: E402
...
STRUCTURAL_ARTIFACTS = DEFAULT_STRUCTURAL_ARTIFACTS
SEMANTIC_ARTIFACTS = ROOT / "training" / "artifacts" / "semantic"
...
DECISION_VERSION = "decision-2026.03-r01"
...
def configure_candidate(structural_artifacts: Path, decision_version: str) -> None:
    """Select versioned candidate paths without touching deployed artifacts."""
    global CANDIDATE_PATH  # noqa: PLW0603
    global DECISION_PERFORMANCE_DIR  # noqa: PLW0603
    global DECISION_RUN_DIR  # noqa: PLW0603
    global DECISION_VERSION  # noqa: PLW0603
    global SIGNALS_META_PATH  # noqa: PLW0603
    global SIGNALS_PATH  # noqa: PLW0603
    global STRUCTURAL_ARTIFACTS  # noqa: PLW0603

    if not re.fullmatch(r"[a-z0-9][a-z0-9._-]{2,79}", decision_version):
        raise ValueError(f"invalid decision candidate version: {decision_version!r}")
    STRUCTURAL_ARTIFACTS = structural_artifacts.expanduser().resolve()
    DECISION_VERSION = decision_version
    run_root = ROOT / "ml_training" / "decision_layer" / "runs" / DECISION_VERSION
    DECISION_RUN_DIR = run_root / "artifacts"
    DECISION_PERFORMANCE_DIR = (
        ROOT / "ml_training" / "decision_layer" / "performance" / DECISION_VERSION
    )
    SIGNALS_PATH = run_root / "cache" / "branch_signals.csv"
    SIGNALS_META_PATH = run_root / "cache" / "branch_signals.meta.json"
    CANDIDATE_PATH = DECISION_RUN_DIR / "fusion_weights.json"
...
def model_fingerprint() -> dict:
    ...
    for name, directory in (
        ("structural", STRUCTURAL_ARTIFACTS),
        ...
        out[name] = {
            "temperature": json.loads(temp.read_text())["temperature"]
            if temp.is_file()
            else None,
    ...
    out["dataset"] = {
        ...
        "image_bytes": sum(
            path.stat().st_size for path in named_images if path.is_file()
        ),
    }
    return out
...
def load_cached_signals() -> pd.DataFrame | None:
    ...
    recorded = (
        json.loads(SIGNALS_META_PATH.read_text())
        if SIGNALS_META_PATH.is_file()
        else None
    )
    if recorded != current:
        ...
        print(
            "  Recomputing rather than fitting fusion to a model that is not installed."
        )
        return None
...
def compute_signals(manifest: pd.DataFrame) -> pd.DataFrame:
    ...
    structural = load_structural(str(STRUCTURAL_ARTIFACTS))
    semantic = load_semantic()
    rows = []
    for i, r in enumerate(manifest.itertuples(), 1):
        with Image.open(MIX_DIR / "images" / r.filename) as opened:
            image = opened.convert("RGB")
        quality = assess_image_quality(image)
        evidence_mode = getattr(r, "evidence_mode", f"gallery_{r.image_class}")
        if evidence_mode == "camera_uncertain_abstain" or not quality.usable:
            p_structural = None
            structural_type = "uncertain"
        else:
            # Structural v3 uses the same measured candidate output for Gallery
            # and usable Camera inputs. Only the explicit quality-abstention cell
            # lacks Structural evidence; no source-specific score is fabricated.
            s = structural.predict(normalize_measured_range(image, quality))
        ...
        [
            build_feature_vector(
                BranchInputs(
                    ...
                    # Not available in QRGuard-Mix; see the module docstring.
                    llm_score=None,
...
def tune_thresholds(
    ...
    """Tune thresholds on training data, including the serving-policy cell gates."""
    ...
    if rows is None or "target_tier" not in rows:
        return safe_max, blocked_min

    # Aggregate-only tuning can legally choose a low Blocked threshold that
    # turns too many clean Open-Wi-Fi scans from Warning into false Blocks. Use
    # the training partition (never the held-out rows) to optimise the same cell
    # contract used by the final evaluation. With only 35 training rows per
    # cell, every strict cell target may occasionally be infeasible at once; in
    # that case minimise total shortfall instead of discarding the cell policy
    # and reverting to an aggregate-only threshold.
    candidates = []
    for candidate_safe in SAFE_SEARCH:
        for candidate_blocked in BLOCKED_SEARCH:
            if candidate_safe >= candidate_blocked:
                continue
            adjusted, tiers = apply_runtime_policy(
                scores, rows, candidate_safe, candidate_blocked
            )
            blocked = tiers == "blocked"
            safe = tiers == "safe"
            precision = float(y[blocked].mean()) if blocked.sum() else float("nan")
            safe_fnr = float(y[safe].sum() / max(y.sum(), 1))
            cell_metrics = _cell_metrics(rows, adjusted, tiers)
            if (
                not np.isfinite(precision)
                or precision < TARGET_BLOCKED_PRECISION
                or safe_fnr > TARGET_SAFE_FNR
            ):
                continue
            cell_shortfall = _cell_gate_shortfall(cell_metrics)
            exact_tier = float((tiers == rows.target_tier.to_numpy()).mean())
            policy_acceptance = float(
                np.average(cell_metrics.acceptance_rate, weights=cell_metrics.n)
            )
            candidates.append(
                (
                    -cell_shortfall,
    ...
    if candidates:
        *_, safe_max, blocked_min = max(candidates)
    return safe_max, blocked_min
...
def apply_runtime_policy(
    ...
    for position, row in enumerate(rows.itertuples()):
        ...
        structural_abstained = pd.isna(getattr(row, "p_structural", np.nan))
        ...
        # Mirror app.pipeline.run_scan: a quality abstention is not attack
        # evidence, but an unknown, flagged, or non-URL result may not silently
        # become Safe. The policy is source-neutral for Structural v3.
        if structural_abstained and adjusted[position] < safe_max:
            p_url_present = not pd.isna(row.p_url)
            known_domain = (
                not pd.isna(row.domain_unknown) and float(row.domain_unknown) == 0.0
            )
        ...
        if (
            structural_abstained
            and not pd.isna(row.p_url)
            and float(row.p_url) >= SEMANTIC_ONLY_BLOCK_MIN
        ):
            adjusted[position] = max(adjusted[position], blocked_min)

        # Mirror FusionEngine's partial-evidence upper guard: a moderate
        # Semantic score alone warrants Warning, not a hard Block.
        if (
            structural_abstained
...
def _cell_evaluation(
    rows: pd.DataFrame, scores: np.ndarray, tiers: np.ndarray
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Build the security-impact metrics shared by tuning and holdout gates."""
    per_cell = pd.DataFrame(
        {
            "cell": rows["cell"].to_numpy(),
            "payload_kind": rows["payload_kind"].to_numpy(),
            "evidence_mode": rows["evidence_mode"].to_numpy(),
            "score": scores,
            "predicted_tier": tiers,
            "target_tier": rows["target_tier"].to_numpy(),
        }
    )
    ...
    return per_cell, metrics


def _cell_metrics(
    rows: pd.DataFrame, scores: np.ndarray, tiers: np.ndarray
) -> pd.DataFrame:
    return _cell_evaluation(rows, scores, tiers)[1]


def _cell_gate_failures(cell_metrics: pd.DataFrame) -> list[str]:
    failures = []
    for cell, row in cell_metrics.iterrows():
        ...
        if (
            cell.endswith("_phishing_url")
            and not cell.startswith(
                (
                    "gallery_tampered_",
                    "gallery_adversarial_",
                    "camera_tampered_consensus_",
                )
            )
            and float(row.blocked_rate) < 0.65
        ):
            failures.append(f"{cell} Blocked recall {row.blocked_rate:.4f} < 0.65")
    return failures


def _cell_gate_shortfall(cell_metrics: pd.DataFrame) -> float:
    """Return summed distance from the cell gates for training-time ranking."""
    shortfall = 0.0
    for cell, row in cell_metrics.iterrows():
        if cell.endswith("_phishing_url") and not cell.startswith(
            (
            ...
            (
                "gallery_clean_",
                "camera_clean_consensus_",
                "camera_uncertain_abstain_",
            )
        ):
            minimum = 0.90
        elif cell.startswith(
            ...
            minimum = 0.90
        shortfall += max(0.0, minimum - float(row.acceptance_rate))
        if cell.endswith("_phishing_url") and not cell.startswith(
            (
                "gallery_tampered_",
                "gallery_adversarial_",
            ...
            shortfall += max(0.0, 0.65 - float(row.blocked_rate))
    return shortfall
...
def ablation(X: np.ndarray, y: np.ndarray, idx_tr, idx_te) -> dict[str, float]:
    ...
    for label, cols in subsets.items():
        ...
        coef, b = fit_constrained(X[np.ix_(idx_tr, cols)], y[idx_tr], bounds=sub_bounds)
...
def write_performance_report(
    *,
    ...
    ablation_frame[["roc_auc", "accuracy"]].plot.bar(
        ax=ax, color=["#d58a52", "#7a4b2a"]
    )
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Score")
    ...
    status = (
        "PASSED; promotion requested"
        if not gate_failures and promotion_requested
        else "PASSED; candidate not promoted"
        if not gate_failures
        else "REJECTED; not deployed"
    )
    threshold_summary = (
        f"Safe < {weights_blob['safe_max']}; "
        f"Warning < {weights_blob['blocked_min']}; "
        f"Blocked >= {weights_blob['blocked_min']}"
    )
...
def main(
    ...
    configure_candidate(structural_artifacts, decision_version)
    if not STRUCTURAL_ARTIFACTS.is_dir():
        sys.exit(f"Structural candidate artifacts not found: {STRUCTURAL_ARTIFACTS}")
    ...
    safe_max, blocked_min = tune_thresholds(
        _predict_proba(X[idx_tr], coef, intercept),
        y[idx_tr],
        signals.iloc[idx_tr].reset_index(drop=True),
    )
    ...
    print(f"Blocked precision    : {blocked_prec:.4f}")
    print(f"Blocked precision target: >= {TARGET_BLOCKED_PRECISION}")
    print(f"Safe-tier FNR        : {safe_fnr:.4f}   (target <= {TARGET_SAFE_FNR})")

    print("\n=== Tier x truth ===")
    print(
        pd.crosstab(
            pd.Series(tiers, name="verdict"),
            pd.Series(np.where(y[idx_te] == 1, "dangerous", "safe"), name="truth"),
        ).to_string()
    )

    target_tiers = held_out["target_tier"].to_numpy()
    print("\n=== Per-cell deployment gates ===")
    per_cell, cell_metrics = _cell_evaluation(held_out, scores_te, tiers)
    ...
    gate_failures.extend(_cell_gate_failures(cell_metrics))
    ...
    weights_blob = {
        ...
        "metadata": {
            ...
            "zero_unidentifiable": [
                n for n, k in zip(FEATURE_NAMES, constant_mask) if k
            ],
            "zero_clamped_by_constraint": [
                n
                for n, c, k in zip(FEATURE_NAMES, coef, constant_mask)
    ...
    if promote:
        ...
        print(f"All gates passed; explicitly promoted weights -> {WEIGHTS_PATH}")
    else:
        print("All gates passed; candidate retained without runtime promotion.")
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=Path,
        default=DEFAULT_STRUCTURAL_ARTIFACTS,
        help="accepted Structural candidate artifact directory to recalibrate against",
    )
    ap.add_argument("--decision-version", default="decision-2026.03-r01")
    ap.add_argument(
        "--promote",
        action="store_true",
        help="replace runtime fusion weights only after every gate passes",
    )
    options = ap.parse_args()
    main(
        options.cache_only,
        options.structural_artifacts,
        options.decision_version,
        options.promote,
    )

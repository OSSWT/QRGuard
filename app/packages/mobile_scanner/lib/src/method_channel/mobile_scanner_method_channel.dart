...
import 'package:mobile_scanner/src/objects/exposure_compensation_state.dart';
...
class MethodChannelMobileScanner extends MobileScannerPlatform {
  ...
  /// The name of the method that queries exposure-compensation capability.
  @visibleForTesting
  static const String kGetExposureCompensationStateMethodName =
      'getExposureCompensationState';

  /// The name of the method that applies an exposure-compensation index.
  @visibleForTesting
  static const String kSetExposureCompensationIndexMethodName =
  ...
  @override
  Future<ExposureCompensationState> getExposureCompensationState() async {
    if (defaultTargetPlatform != TargetPlatform.android) {
      throw UnimplementedError(
        'Exposure compensation is only implemented for Android.',
      );
    }
    final response = await methodChannel.invokeMapMethod<Object?, Object?>(
      kGetExposureCompensationStateMethodName,
    );
    return ExposureCompensationState.fromMap(response ?? const {});
  }
  ...
  Future<int> setExposureCompensationIndex(int index) async {
    ...
    final applied = await methodChannel.invokeMethod<int>(
      kSetExposureCompensationIndexMethodName,
      index,
    );

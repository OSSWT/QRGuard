...
class MobileScannerController extends ValueNotifier<MobileScannerState> {
  ...
  /// Query CameraX exposure-compensation support for the running camera.
  Future<ExposureCompensationState> getExposureCompensationState() async {
    _throwIfNotInitialized();
    if (!value.isRunning) {
      return const ExposureCompensationState(
        supported: false,
        currentIndex: 0,
        minimumIndex: 0,
        maximumIndex: 0,
    ...
    return MobileScannerPlatform.instance.getExposureCompensationState();
  }

  /// Apply an exposure-compensation index, clamped to the device range.
  Future<int> setExposureCompensationIndex(int index) async {
    _throwIfNotInitialized();
    if (!value.isRunning) return 0;
    final state = await getExposureCompensationState();
    if (!state.supported) return state.currentIndex;
    return MobileScannerPlatform.instance.setExposureCompensationIndex(
      state.clampIndex(index),
    );
  }

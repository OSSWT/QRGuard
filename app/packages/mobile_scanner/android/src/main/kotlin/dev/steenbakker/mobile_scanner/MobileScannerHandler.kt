...
//            ))
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "getExposureCompensationState" -> getExposureCompensationState(result)
            "setExposureCompensationIndex" -> setExposureCompensationIndex(call, result)
    ...
    private fun getExposureCompensationState(result: MethodChannel.Result) {
        try {
            result.success(mobileScanner?.getExposureCompensationState())
        } catch (e: ZoomWhenStopped) {
            result.error(
                MobileScannerErrorCodes.GENERIC_ERROR,
                "Cannot query exposure when camera is stopped.",
                null
            )
        }
    }

    private fun setExposureCompensationIndex(
        call: MethodCall,
        ...
        if (index == null) {
            result.error(
                MobileScannerErrorCodes.GENERIC_ERROR,
                "Exposure compensation index is required.",
                null
            )
            return
        }
        try {
            val future = mobileScanner!!.setExposureCompensationIndex(index)
            future.addListener({
                try {
                    ...
                    result.error(
                        MobileScannerErrorCodes.GENERIC_ERROR,
                        "Could not apply exposure compensation.",
                        e.localizedMessage
                    )
                }
            ...
            result.error(
                MobileScannerErrorCodes.GENERIC_ERROR,
                "Cannot set exposure when camera is stopped.",
                null
            )
        }
    }

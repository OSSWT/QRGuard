...
//    }
    ...
    fun setFocus(x: Float, y: Float) {
        ...
        // The one-argument builder meters AF, AE and AWB on the same detected
        // QR region. AF-only left bright screen codes exposed against the room.
        val action = FocusMeteringAction.Builder(afPoint).build()

        cam.cameraControl.startFocusAndMetering(action)
    }

    fun getExposureCompensationState(): Map<String, Any> {
        val cam = camera ?: throw ZoomWhenStopped()
        val state = cam.cameraInfo.exposureState
        return mapOf(
            "supported" to state.isExposureCompensationSupported,
            "currentIndex" to state.exposureCompensationIndex,
            "minimumIndex" to state.exposureCompensationRange.lower,
            "maximumIndex" to state.exposureCompensationRange.upper,
            "stepEv" to state.exposureCompensationStep.toFloat().toDouble(),
        )
    }

    fun setExposureCompensationIndex(index: Int) =
        (camera ?: throw ZoomWhenStopped())
            .cameraControl
